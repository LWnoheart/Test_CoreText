...
-(NSArray *)getAttImageWithImageSize:(CGSize)imageSize referenceFont:(UIFont *)referenceFont
{
    // 1.用来保存图片对象的数组
    ...
    // 3.遍历获取到的图片
    [regex enumerateMatchesInString:self.string options:NSMatchingReportCompletion range:NSMakeRange(0, self.string.length) usingBlock:^(NSTextCheckingResult * _Nullable result, NSMatchingFlags flags, BOOL * _Nonnull stop) {
        // 3.0获取正则查找出来的内容
